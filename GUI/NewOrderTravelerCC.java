 ...
 */
    ...
    /**
    *this method is launched when the user presses GetAnotherDate button
	 * this method opens AvailibilityTable page while also closing the current page
     * @param event -javaFx ActionEvent,the event that started this method 
     */
    @FXML
	void actionGetAnotherDateBtn(ActionEvent event) {
    	try {
    		anPane_anchore.getChildren().setAll((Node) FXMLLoader.load(getClass().getResource("AvailibilityTableGUI.fxml")));				
		} catch (Exception e) {
			e.printStackTrace();
    ...
    /**
     *this method is launched when the user presses EnterWaitingList button
 	 * this method put a pop up on the screen and updates the waiting list
      * @param event -javaFx ActionEvent,the event that started this method 
      */
	@FXML
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 /**
     *this method is launched when the user pressesOrder button
 	 * this method checks if the order is good to go,if yes the method updates the orders table 
      * @param event -javaFx ActionEvent,the event that started this method 
      */
	@FXML
	void actionOrderBtn(ActionEvent event) {
		if (checksForTexts == true && newOrder != null) {
			...
			if (!existOrder) {
				...
				if(Math.abs(ChronoUnit.DAYS.between(LocalDate.now(), LocalDate.of(visitDate.getYear()+1900, visitDate.getMonth()+1, visitDate.getDate())))==1&&visitTime.compareTo(Time.valueOf(LocalTime.now()))<=0)
					newOrder.setOrderStatus("confirmed");
				if(Math.abs(ChronoUnit.DAYS.between(LocalDate.now(), LocalDate.of(visitDate.getYear()+1900, visitDate.getMonth()+1, visitDate.getDate())))==0&&visitTime.compareTo(Time.valueOf(LocalTime.now()))>=0)
					newOrder.setOrderStatus("confirmed");
				if(Math.abs(ChronoUnit.DAYS.between(LocalDate.now(), LocalDate.of(visitDate.getYear()+1900, visitDate.getMonth()+1, visitDate.getDate())))==0&&visitTime.compareTo(Time.valueOf(LocalTime.now()))<=0)
				{
					newOrder=null;
				...
				if (OrderCC.addOrder(newOrder)) {
					...
					popUpCC aa =new popUpCC();
					aa.start(new Stage(), true, "We are Waitting For You", "Your Order has been placed successfully!", "Ok , Great!");
					try {
						mainOrderPage_anchore.getChildren().clear();
						mainOrderPage_anchore.getChildren().setAll((Node) FXMLLoader.load(getClass().getResource("VisitorHomePage.fxml")));
					} catch (IOException e) {}					
				} else {
					try {
						error_message_label.setVisible(false);
						mainOrderPage_anchore.getChildren().clear();
						mainOrderPage_anchore.getChildren().setAll((Node) FXMLLoader.load(getClass().getResource("unconfirmedGUI.fxml")));												
					} catch (Exception e1) {
					}
				}
			} else { error_existOrder_label.setVisible(true);	}
		} else { error_message_label.setVisible(true); 	}
	}
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	/**
	 * this method runs at the beginning
	 * this method initialize the labels on the screen
	 */
    @Override
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			if (VisitorHomePageCC.subscriber_private != null) {
				...
				if(GoNatureClient.currentTraveler.isGuide() == true) {
    ...
    /**
     *this method is launched when the user finishes putting all his details in the order
      * @param event -javaFx MouseEvent,the event that started this method 
      */
    @FXML
    void mouseEventCalcPrice(MouseEvent event) {calculateThePrice(null);}
    
    
    /**
     *this method is calculates the price of the order
     *this method updates the price label depinding on the order details
      * @param event -javaFx ActionEvent,the event that started this method 
      */
	@FXML
	void calculateThePrice(ActionEvent e) {
		...
		try {
			...
			Date date = new Date(date_order_datePicker.getValue().getYear() - 1900,
					             ...
					             date_order_datePicker.getValue().getDayOfYear());
								...
								if (minutes_order_comBox.getValue() != null) {
									if (isValid(email)) {
										...
										if(payNow_checkBox.isSelected() && checkVisaTexts()) {
											...
											if(payNow_checkBox.isSelected()) {
												newOrder = new Order(parkOrder, date, time, numberOfVisitors, email, id,
														orderType, price, "waiting", false, false, telephoneNumber);
	...
	/**
	 * this method checks if the telephone is correct,if yes the method returns true,otherwise returns false
	 * @param id
	 * @return boolean
	 */
	...
	/**
	 * this method checks if the email is correct,if yes the method returns true,otherwise returns false
	 * @param email
	 * @return boolean
	 */
	...
	/**
	 * this method checks if the Visa is correct,if yes the method returns true,otherwise returns false
	 * @return boolean
	 */
	private boolean checkVisaTexts() {
		...
		if (cvv.length() ==3 ) {// checking length of id number,should be 3		
			for (int i = 0; i < cvv.length(); i++)
				if (!Character.isDigit(cvv.charAt(i)))// if one character is not a digit then id isn't valid return false
					{
					    return false;
					}			
		}else {
		...
		if (ed.length() >3 && ed.length() <7) {// checking length of id number,should be 5 mixmum		
			for (int i = 0; i < ed.length(); i++)
				if (!Character.isDigit(ed.charAt(i)) && ed.charAt(i) !='/')// if one character is not a digit then id isn't valid return false
					{
					    return false;
					}			
		}else {
		...
		if (ccn.length() >8 && ccn.length() < 20) {// checking length of id number,should be 9		
			for (int i = 0; i < ccn.length(); i++)
				if (!Character.isDigit(ccn.charAt(i)))// if one character is not a digit then id isn't valid return false
					{
					    return false;
					}			
		}else {
