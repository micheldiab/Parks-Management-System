...
/**
 * this class works when there isn't any left space in the park and the visitor wants to choose another order time
 * @author Group2
 *
 */
 ...
 */
	...
	void choosingRableViewDateandTime(MouseEvent e) {		
			if(choosingAnotherDATETIME_TableView.getSelectionModel().getSelectedItem() != null) {
				azaza =choosingAnotherDATETIME_TableView.getSelectionModel().getSelectedItem();				
			}
	}	
	
	/**
	 * this methods is launched when the user chooses another order time 
	 * this methods opens a pop up on screen
	...
	void actionchoosingAnotherOrder(ActionEvent event) {
		...
		try {
    	    ...
    	    if(choosingAnotherDATETIME_TableView.getSelectionModel().getSelectedItem() != null) {
    	    	changeOrder.setVisitDate(azaza.getVisitDate());
    	    	changeOrder.setVisitTime(azaza.getVisitTime());
    	    System.out.println(azaza.getVisitTime()+"  "+azaza.getVisitDate());
	 ...
	 * this method initialize the tableView of the relevant orders time and date
	 */
	@Override
	public void initialize(URL arg0, ResourceBundle arg1) {		
		ArrayList<Timestamp> array = new ArrayList<Timestamp>();
		array = OrderCC.fetchAvailibleOrderDates(NewOrderTravelerCC.newOrder.getVisitTime(), NewOrderTravelerCC.newOrder.getVisitDate(),NewOrderTravelerCC.newOrder.getNumOfVisitors(), NewOrderTravelerCC.newOrder.getPark().getName());
